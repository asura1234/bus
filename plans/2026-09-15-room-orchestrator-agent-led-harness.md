...
- **当前源码已验证**：`message.send` 每次都创建新的 Prompt/Request 并进入 agent FIFO；当前没有与 active request/provider turn 绑定的 steering operation，因此 mid-work pivot 会被误表达为下一份 assignment。按决策 17，MVP 不提供 steering：pivot 是 Orchestrator 显式发送的新 durable `SendMessage` Request。
- **开发者明确决定（2026-09-16 notification/round-trip）**：generic Bus CLI/Worker 只维护一份 durable settlement SOT。`message.send` 持久化入队后立即返回 message/request identity；`message.status` 与 bounded `wait` 查询同一 persisted Request/message settlement，CLI waiter 退出或重启后仍可继续查询；Worker 在同一次 durable Request settlement 中记录 `RequestSettled` semantic fact 并唤醒 room Orchestrator。Harness、content 与 UI 不得新增 polling loop、terminal completion classifier、workflow-specific notifier 或第二套 completion state；未来若加入 `send --wait`，也只能组合既有 send + wait，不能建立第二个状态机。只有 Orchestrator model 解释 reply 并选择下一步。
...
- **当前源码已验证（`7bc910f2`，计划执行前）**：当前 `@` menu 只有 `All` 和 terminal agents；room header 仍以 notes editor 为中心，Goal/Non-goals、proposal confirmation 与 workflow approval UI 都不存在。
- **Task 2 generation 2 round-01 task review 已验证（2026-09-16）**：Task 1 已交付 typed `ConfirmRoomBriefProposal`、`CreateDeveloperWorkflowApproval`、`RoomBriefConfirmationReceipt` 与 `BusState::work_settlement`，UI 已按类型消费。缺口是：`BusCommand` 没有 Human→Orchestrator room message，`@Orchestrator` 只是未派发的 client flag，Orchestrator-only send 得到 `NoRecipients`；workflow base digest/rendered diff 只在 `workflow_drafts.rs::developer_approval_request` 内部计算，draft 也不绑定 target `WorkflowId`；`RetireParticipant`/`ReplaceParticipant`/`RequestHuman` 落入 `runtime_orchestrator.rs` 的 generic unavailable fallback；`OrchestratorState` 没有 human-task/attempt/lesson 状态。UI 因此伪造了把 `intent_digest` 当 JSON 的 review、硬编码 `HumanTaskId(0)` no-op、`serde_json::Value` lease 行、digest-as-text lifecycle 面板、点击时重算的 proposal digest 与 `Abandoned`→`wedged` 标签，测试以 JSON patch 伪造 Worker 状态。
- **开发者明确决定（2026-09-16 Task 2 plan repair）**：保留 locked goal 与 agent-led design，只补 objective atomic seams，并删除或延期没有 objective Worker fact 的 UI/state/operation 要求；详见归档决策 16。
...
- **开发者明确决定**：Orchestrator 必须知道 requirements、locked Goal/Non-goals、active SOP、agent assignment/status、durable attempts/lessons；它不应吸收源码与技术实现细节，codebase 与技术问题通过 message 转发给相关 coding agent（按决策 18，Orchestrator 没有 codebase outline 或 artifact body query）。
...
flowchart LR
    ...
    O <--> B
    C <--> B
    B --> FS[bounded workflow/content reads]
...
- Generic notification/round-trip 仍属于 Bus CLI/Worker capability settlement，而不是 workflow logic：send durable enqueue 后立即返回 identity，wait/status 读取同一 persisted settlement，`RequestSettled` 只作为 semantic fact 唤醒 Orchestrator。UI、content、acceptance harness 与 model loop 不得复制 settlement、轮询 terminal 判断完成或据此自动推进；optional send-with-wait 只是同一 send + wait primitive 的 convenience composition。
...
- 发送/retry（新的 durable `SendMessage` Request）/abandon-idle-request/revise-SOP/lease/permission-once/向 Human 求助等 workflow 选择必须有对应的 model-authored tool call；Human 可通过相同 typed operation 显式授权 approved recovery。FIFO 投递、hook readiness、callback settlement、operation receipt、assignment verification、lease epoch/expiry bookkeeping 与 semantic-fact wake 只是既有授权的 capability settlement/fact exposure，不是新的 workflow 决策。
...
- Human-only 模式使用同一 room UI 与 Human control capabilities，但没有隐藏自动执行；配置 key 后，新 room 默认启用 AI orchestrator。
...
5. **Technical-work boundary**
   ...
   - **选择**：第三项。Harness 不暴露 shell、source write、test 或 Git；文件读取仅为 bounded workflow draft/content bundle reads（决策 18 删除了 codebase outline 与 artifact body query）。Codebase 与技术问询通过 Bus message 转发 coding agent。
...
7. **Durable recovery model**
   ...
   - **选择**：第三项。保存 settled messages（包括 participant 撰写的 attempt、lesson 与 human-help 内容）、tool intent/result、approvals、artifact evidence 和 leases；attempt、lesson 与 human task 不是独立 typed state（见决策 16）；context 是可重建 projection，不保存任何程序拥有的 next node。Artifact existence/hash 是独立证据，不自动完成 assignment 或选择 progression。
...
8. **Participant model**
   ...
   - **选择**：第三项。Bus 只允许一个 Human；Human、Orchestrator、coding Agent 都有 stable `ParticipantId`，都能作为 message author/recipient、接收 assignment、产生 evidence/status 并保留 history。Orchestrator 不是 `Provider`，也不进入 coding-agent PTY `AgentRecipients` FIFO；Human↔Orchestrator 走 room message/wake（Human→Orchestrator 由 Task-1-owned Worker command 把 exact body 写入 durable room context 并产生 semantic wake，不创建 Prompt/Request），coding Agent assignment/reply 走现有 Request/FIFO 并保留 author/work correlation。`All` 明确只选 coding agents。
...
9. **Persistence and mutation authority**
   ...
   - **选择**：第三项。Bus Worker 是 room、participant、delivery、coordination、logical append-only journal 与 operation ledger 的唯一 mutation writer；同一 versioned durable document 原子保存 journal entries、domain state 与 derived indexes。`RoomBrief`、approval、workflow promotion review、lease、UI/context projection 都由 journal revision 或 Worker-authoritative state 派生，不存在第二份可独立修改的 SOT。Provider HTTP/model loop 在 coordinator thread 外；UI、CLI、model tools 都通过同一 typed command boundary。External effect 前先保存 immutable intent；effect 后由 Worker 保存 result/receipt 与 domain mutation。Crash 落在二者之间时只暴露 `uncertain/reconcile-needed` 事实，runtime 不自动重发或选择恢复动作。
...
15. **Generic notification 与 reply round-trip source of truth**
   - **选项**：由 harness/UI 轮询 terminal 并自行分类 completion；由 workflow/content 建立专用 notifier；复用 Bus CLI/Worker 的单一 durable Request/message settlement 与 semantic wake。
   - **选择**：第三项。Send 只做 durable enqueue 并立即返回 message/request identity；wait/status 只读取同一 persisted settlement，且等待进程退出或重启不影响后续查询；Worker 在该 Request 的同一次 durable settlement 中追加 `RequestSettled` fact 并唤醒 room Orchestrator。Future `send --wait` 若存在，只能顺序组合同一 send + wait contract，不能新增 state machine 或 completion SOT。
   - **依据**：开发者对 Task 1 generation 4 的直接决定与已验收实现；notification 是 generic Bus capability settlement，不是 harness workflow logic。只有智能 Orchestrator 解释 reply 并决定 progression，Task 2/3、successor content 与 UI 均不得重建 polling/classification/notifier 逻辑。

16. **Task 2 review 后的 UI/Worker seam 简化**
   - **选项**：由 Task 1 补齐 human-task、attempt/lesson、lifecycle provenance、lease 与 to-do typed state/projection/operations，再由 UI 全量渲染；保留现有 UI，用 JSON 或 digest 字段承载缺失事实；只补 objective atomic seams，其余交给 agent judgment、ordinary room messages 与 existing human Bus actions。
   - **选择**：第三项。Task 1 只新增两个缺失的 Worker seam：
     - `BusCommand::MessageOrchestrator(HumanOrchestratorMessage)`：把 Human 写的 exact body 持久化为 Human-authored、Orchestrator-addressed room message，并在同一 durable save 中产生 deduplicated `RoomWake::HumanMessage`；不创建 Prompt/Request，也不进入 `AgentRecipients` FIFO。
     - Draft create 时绑定 immutable 的 optional canonical target `WorkflowId`；Worker 用 `create_approval` 重算所用的同一函数产出 typed `WorkflowPromotionReview`（developer、draft ID/revision/content digest、workflow ID、current base digest、exact rendered diff 与 reviewed diff digest）。

     其余 UI facts 只经 typed `BusState` reads 读取。Task 2 负责四件事：派发 exact displayed proposal revision+digest；从 typed review projection 派发 approval；把 `Abandoned` 标为 abandoned/recovered 而不是 wedged；删除所有 fake UI paths。

     以下内容及其 acceptance requirements 全部删除或延期：structured attempt/lesson state/UI、human-task state/buttons、`RetireParticipant`/`ReplaceParticipant`/`RequestHuman` operations、lifecycle provenance panels、lease/to-do panels。Attempt、lesson 与 human help 改为 participant 撰写的 ordinary room messages；移除 agent 沿用 existing Human Bus agent delete action；lease 仍是 Task 1 typed operation，但没有 UI 面板。`RequestSettled` 仍是唯一 completion SOT。
   - **依据**：开发者 2026-09-16 对 Task 2 round-01 `PLAN_REPAIR_REQUIRED upstream-contract` review 的直接决定。Review 证明这些面板在真实 Worker 状态下没有事实来源，迫使 UI 伪造 stringly JSON、digest-as-payload 与 synthetic Worker state。它们表达的是 agent judgment，或可由 ordinary message 承载的内容，不是 harness 必须拥有的 objective fact、hard safety invariant 或 durable identity/settlement。

17. **Orchestrator operation surface 只保留 runtime 已实现的 atomic operations**
   - **选项**：Task 1 补实现 `CreateAgent`、`SteerActiveWork`、`SuspendWork`、`ResolveQueuedWork` 与 `UpdateCoordination`；保留它们，但 runtime 返回 unavailable；从 closed contract 删除，由 existing Human Bus actions 与 durable messages 覆盖。
   - **选择**：第三项。Closed `RoomOperation` 只含 `SendMessage`、`AbandonIdleRequest`、`PersistWorkflowDraft`、`PromoteWorkflowDraft`、`AcquireResource`、`ReleaseResource` 与 `ApprovePermissionOnce`（决策 19 另增 `ProposeRoomBrief`，共 8 个）。上述五个 variant 从 enum、provider schema、capability map、Worker dispatch、manual assertions 中删除。MVP 只协调 pre-existing room agents：创建和删除 agent 沿用 existing Human Bus action。Orchestrator 通过 durable `SendMessage`/new Request、`RequestSettled` wake、typed reads、permission-once 与 exact idle-Request abandonment 来适应现实；不引入 replacement state machine、steering 或 queued-work disposition。
   - **依据**：开发者 2026-09-16 的 anti-overengineering 决定。PLAN_REPAIR concern 2 证明 runtime 的 generic unavailable fallback 没有实现这些 variants，closed registry 不应 advertise 必然失败的 tool。

18. **Orchestrator query surface 只保留 runtime 已实现的 typed reads**
   - **选项**：Task 1 补实现 `ReadCodebaseOutline` 与 `ReadArtifact`；保留它们，但 runtime 返回 unavailable；从 closed contract 删除，把 artifact evidence 留在 `WorkSettlement`，并把 codebase/技术 inspection 委托给 coding agents。
   - **选择**：第三项。Closed `RoomQuery` 只含 `InspectWork`、`WaitForChange`、`ReadAgent`、`ObservePermissionPrompt`、`ReadWorkflowDraft` 与 `ReadContent`。`ReadCodebaseOutline`、`ReadArtifact` 与 `OutlineCursor` 从 enum、provider schema、capability map、Worker dispatch、Task 1 constraints/gates 与 test plan 中删除。Artifact evidence 继续通过 `WorkSettlement.artifacts` 暴露；Orchestrator 需要 codebase 或技术事实时，以 durable `SendMessage` 委托相关 coding agent。`ReadContent` 继续承担 bounded approved SOP/content 读取，`ReadWorkflowDraft` 读取 Bus-owned drafts。本决策取代此前“有限 codebase outline”要求。
   - **依据**：开发者 2026-09-16 的 anti-overengineering 决定。Runtime 对这两个 query 只返回 generic unavailable；closed registry 不应 advertise 必然失败的 tool，而低成本 Orchestrator 本就不应吸收源码细节。

19. **Room Brief proposal producer**
   - **选项**：给 test harness 加 test-only `set_room_brief` seam；恢复通用 `UpdateCoordination`；新增一个 closed、pathless、typed `ProposeRoomBrief` operation。
   - **选择**：第三项。`RoomOperation::ProposeRoomBrief { expected_revision, goal, non_goals }` 复用 `Coordinate` capability，room 与 Orchestrator author 由 Worker 注入。Worker 的处理如下：
     - 要求 room 未锁定，且 `expected_revision` 等于当前 brief revision；
     - 要求 goal 与 non-goals 非空白；两者大小只受既有 Bus command/control frame 与 serialization limits 约束（这些限制已约束所有 typed operations），不新增 per-field cap；
     - 自行分配下一 revision，经单一 Worker writer durable 保存为未确认 proposal，并通过既有 `room_brief_proposal` typed read 发布；
     - stale、已锁定或空白的输入 typed-reject，且零 Room Brief mutation。

     它从不自动确认、不锁定 brief、不选择 workflow step；只有 Human 的 `ConfirmRoomBriefProposal` 能锁定。Human 对 brief 的修改请求是 ordinary `MessageOrchestrator` message。Closed operation 集合由 7 个增至 8 个，query 集合保持 6 个。Room history 按既有共享、单调分配的 message/prompt identity，稳定交错 Human↔Orchestrator room messages 与 prompt-grouped Request history；不新增 timestamp 或第二个 ordering store。
   - **依据**：开发者 2026-09-16 对 Task 2 generation 3 owner gap 的直接决定。删除 `UpdateCoordination` 后，production 与 test harness 都没有 Room Brief proposal producer，proposal confirmation UI 缺少真实事实来源；test-only seam 会伪造 production 不存在的路径。该 operation 是原子 capability，不是 workflow logic。

20. **删除 synthetic Task 3**
   - **选项**：为 real-process acceptance 新增 production 可达的 fake provider、fake terminal transport、fake participant 与 Human dev-control 注入面；在 Python 中模拟这些 seam；删除 Task 3。
   - **选择**：第三项。删除 Task 3、其两个 orchestrator acceptance 脚本与 justfile owner、maintenance-test 追加、real-process scripted fake-provider gate、evidence counters 与 synthetic failure scenarios；不以其它 harness、runner、scenario engine 或新任务替代。本计划仍不发起 live paid DeepSeek call；Task 1 与 Task 2 的 focused gates 及其 in-process fake-provider/typed seam 测试保持不变。
   - **依据**：开发者 2026-09-16 决定。Task 3 generation 1 的 owner gap 证明该场景需要 production test injection surface，违背减法设计。本次 live Bus 执行已在 Orchestrator 判断下实际经历 durable send/wait/status settlement、`RequestSettled` notification、stalled-request abandonment/recovery、agent handoff、implementation/review retry 与 plan repair；这些 live 事实只是执行证据，不是可复现 harness，也不是 completion SOT。
...
| inspect/wait room facts、durable `SendMessage`/new Request、lease、bounded workflow draft/content read、Human-addressed room message | 允许使用 human UI/CLI；创建/删除 agent 沿用 existing Human Bus agent add/delete action | 在 approved room/scope 内允许；不能创建、删除、steer 或 suspend agent | 默认拒绝 process mutation；只回复 assignment 并发布技术 evidence |
...
| locked Goal/Non-goals、authority、destructive/publish action、material budget、project-hook consent、capability grant/revocation | human-only | 只能提出 revision-bound proposal 或向 Human 发送 room message | 只能提出 evidence/request |
...
Risky/unknown permission、missing current Request/turn、任何 stale identity/revision/digest 或 replayed fingerprint 都返回 typed denial 和可安全公开的 current facts 给 Orchestrator，且绝不发送 keys；只有 Orchestrator 后续 model-authored 的 Human-addressed `SendMessage` 才向 Human 求助；本计划没有 human-task typed state。Orchestrator 不创建、删除、steer 或 suspend coding agent。Agent 由 Human 通过 existing Bus agent add/delete action 管理，其 provider、model/effort、root 与 hook consent 沿用现有 launch path。
...
- **新文件**：`src/bus/workflow_drafts.rs`
  - **用途**：实现 Bus Worker-owned workflow repository persistence，而不是 model filesystem tool。Draft operation 只接受 `WorkflowDraftMutation` 与 bounded UTF-8 Markdown；create 由 Bus 发放 opaque `WorkflowDraftId`，并可绑定 immutable canonical target `WorkflowId`（`None` 表示 room-local draft，没有 promotion review）；revise 必须 compare exact revision，且不能改变 target。Owner canonicalize approved root 后从 room ID 与 draft ID 固定派生 `.bus/temp/<room-id>/<draft-id>.md`。Promotion operation 只接受 `ApprovedWorkflowPromotion` reference；separate Human review surface 必须通过 Task-1-owned `CreateDeveloperWorkflowApproval` Worker command 建立 immutable `DeveloperWorkflowApproval`，不得直接调用 store 或在 UI 保存 approval。该 command exact-bind Worker-injected developer identity、draft ID/revision/content digest、canonical `WorkflowId`、current `.bus/standard` base digest 与 exact reviewed diff digest；Worker 必须对 authoritative draft/current base/rendered diff 重算并 compare，durably 保存 approval 与 receipt 后才向 UI 返回成功。Worker 在后续 promotion settlement 时再次 exact-match 全部事实、single-use consume approval，再派生 `.bus/standard/<workflow-id>.md` atomic replace。两条路径都拒绝 caller path/name、absolute/relative traversal、symlink/reparse/case escape、non-regular target、超过 128 KiB、invalid SOP structure 与 stale revision/base/approval；任何验证/IO 失败都不产生 partial file or journal success，且没有 source/test/plan/unrelated-doc target branch。Worker 为每个带 target 的 current draft revision 产出 typed `WorkflowPromotionReview`：计算函数与 `create_approval` 重算所用的 authoritative 函数相同，包含 Worker-injected Human developer、draft ID/revision/content digest、workflow ID、current standard base digest、exact rendered full-replacement diff 与该 diff 的 digest。Draft persist、approval 创建或拒绝、promotion settlement 以及 Worker start/resume 时，Worker 把它刷新到 published snapshot。`approval_command()` 必须逐字段等于 `create_approval` 的重算结果。Projection 不以 JSON 字符串、operation kind 或 intent digest 承载；standard base 在外部被改变后，由旧 projection 派生的 command 只会得到 typed `StaleApproval`。
  ```rust
  pub enum WorkflowDraftMutation {
      Create { workflow_id: Option<WorkflowId>, document: WorkflowDraftDocument },
  ...
  pub struct CreateDeveloperWorkflowApproval {
      pub room_id: RoomId,
      pub expected_developer: DeveloperIdentity,
      pub draft_id: WorkflowDraftId,
      pub draft_revision: u64,
      pub content_digest: Digest,
      pub workflow_id: WorkflowId,
  ...
  pub struct WorkflowPromotionReview {
      pub room_id: RoomId,
      pub expected_developer: DeveloperIdentity,
      pub draft_id: WorkflowDraftId,
      pub draft_revision: u64,
  ...
  impl WorkflowPromotionReview {
      pub fn approval_command(&self) -> CreateDeveloperWorkflowApproval;
  }

  impl WorkflowDraftStore {
      // create_approval recomputes this same value and compares it with the command.
      fn promotion_review(&self, ledger: &WorkflowDraftLedger, room: RoomId, draft_id: &WorkflowDraftId)
...
- **修改文件**：`src/bus/model.rs`、`src/bus/store.rs`、`src/bus/diagnostics.rs`
  - **用途**：first-class participant/message authorship、stable participant/runtime-incarnation lineage、logical append-only journal、operation ledger、semantic-revisioned work settlement、trusted assignment active identity 与 backward-compatible persisted state migration。One versioned document is authoritative；context、UI projection 与 indexes 都是可重建 projection，不保存 model-owned next step。Worker-authoritative Room Brief projection includes the exact current model proposal revision and canonical content digest；Task-1-owned `ConfirmRoomBriefProposal` exact-matches Human identity、room、proposal revision/digest，then in one durable save sets approved revision/locked state and records a replay-safe receipt。UI cannot write `RoomBrief` or treat chat text/local state as confirmation。In one durable save，Task-1-owned `MessageOrchestrator` persists one Human-authored、Orchestrator-addressed room message with the exact submitted body and one deduplicated `RoomWake::HumanMessage`；it changes no Prompt、Request、FIFO or work settlement，and the context builder reads that body from the journal。`BusState` exposes typed `pub(crate)` reads for every Task 2 UI fact，so no consumer parses serialized Worker state。`room_messages` yields records in ascending `RoomMessageId`。Room message IDs and prompt IDs come from the same monotonic `BusState` identity allocator，so Human↔Orchestrator room messages interleave stably with prompt-grouped Request history by that shared identity；no timestamp or second ordering store is added。For a locked-room coding Request，Worker persists the outer frame as part of the same `Prompt` record before delivery；`Prompt::rendered_payload` is the sole serializer for both terminal bytes and callback matching，so the first framed `PromptStarted` must match exactly and only a later same-session/new-turn mismatch can qualify as a trusted continuation。
  ```rust
  ...
  pub struct ConfirmRoomBriefProposal {
      pub room_id: RoomId,
      pub expected_developer: DeveloperIdentity,
      pub expected_proposal_revision: u64,
      pub expected_proposal_digest: Digest,
  }

  pub struct RoomBriefConfirmationReceipt {
  ...
  pub struct HumanOrchestratorMessage {
      pub room_id: RoomId,
      pub body: String,
  }

  pub struct HumanOrchestratorMessageReceipt {
      pub room_id: RoomId,
      pub message_id: RoomMessageId,
  }

  impl BusState {
      pub(crate) fn room_brief_proposal(&self, room: RoomId) -> Option<(u64, Digest)>;
      pub(crate) fn room_brief_confirmation(&self, room: RoomId) -> Option<&RoomBriefConfirmationReceipt>;
      pub(crate) fn workflow_promotion_reviews(&self, room: RoomId) -> &[WorkflowPromotionReview];
      pub(crate) fn developer_workflow_approvals(&self, room: RoomId) -> Vec<&DeveloperWorkflowApproval>;
      pub(crate) fn room_messages(&self, room: RoomId) -> impl Iterator<Item = &RoomMessage>;
      pub(crate) fn work_settlement(&self, work: WorkId) -> Option<WorkSettlement>;
  }
...
- **修改文件**：`src/bus/runtime.rs`、`src/bus/runtime_commands.rs`、`src/bus/runtime_control.rs`、`src/bus/runtime_control_tests.rs`、`src/bus/runtime_callbacks.rs`、`src/bus/runtime_resume.rs`、`src/bus/runtime_tests.rs`、`src/bus/runtime_resume_tests.rs`、`src/bus/runtime_focus_tests.rs`、`src/bus/callbacks.rs`、`src/bus/cursor_reply.rs`、`src/bus/control.rs`、`src/bus/control_cli.rs`、`src/bus/launch.rs`、`src/bus/entry.rs`、`src/bus/resume_launch.rs`、`src/bus/transport.rs`
  ...
  pub enum HumanCoordinationCommand {
      ConfirmRoomBriefProposal(ConfirmRoomBriefProposal),
      CreateDeveloperWorkflowApproval(CreateDeveloperWorkflowApproval),
      MessageOrchestrator(HumanOrchestratorMessage),
  }

  pub enum HumanCoordinationReceipt {
      RoomBriefConfirmed(RoomBriefConfirmationReceipt),
      WorkflowApprovalCreated(DeveloperWorkflowApproval),
      OrchestratorMessageRecorded(HumanOrchestratorMessageReceipt),
  }
...
- **修改目录**：`src/client/shell/bus/`
  - **用途**：Settings 的 Orchestrator agent section（masked key）、author-attributed room history、`@Orchestrator`/separator/`All` exclusion、semantic work-settlement labels、room Goal/Non-goals proposal confirmation，以及 canonical standard-workflow diff review 与 exact developer approval。

    该目录只消费 Task 1 Worker commands 与 typed `BusState` reads。它不读取或解析 `DurableOperation.kind`/`intent_digest`，不把 `BusState` 序列化成 `serde_json::Value` 再取字段，也不硬编码 Worker ID。各交互规则如下：
    - **Composer**：选中 `@Orchestrator` 时，以 submitted exact body 派发 `BusCommand::MessageOrchestrator`；只有选中至少一个 coding agent 时才派发既有 agent `Submit`。Orchestrator-only send 不经过 agent FIFO。`@Orchestrator` toggle 是 composer input，不要求跨 restart 保留；durable fact 只有派发后的 Worker room message。
    - **Proposal**：render 时把 Worker projection 的 exact `(revision, proposal_digest)` 放进 action；dispatch 原样发送 `ConfirmRoomBriefProposal`，不从点击时的 snapshot 重算。Worker 返回 typed stale rejection 后，UI 只显示 stale 并要求重新审阅。
    - **Workflow review**：渲染 typed `WorkflowPromotionReview`，并以其 `approval_command()` 派发 `CreateDeveloperWorkflowApproval`；chat text 不能批准。
    - **Settlement label**：从 `WorkSettlement` 穷尽映射；`Abandoned` 显示为 abandoned/recovered，UI 不推导 `wedged`。

    Structured attempt/lesson、human-task、retire/replace provenance、lease 与 to-do 面板均不存在。拒绝 proposal、求助或汇报 attempt 都是 ordinary room message。UI 不直接调用 `set_room_brief`/`WorkflowDraftStore`，不持久化 authoritative confirmation/approval，也不创建 settlement poller/notifier/completion classifier。Command pending state 仅用于交互反馈；最终状态与 replay-safe receipt 都来自 Worker snapshot/event。
  ```rust
  pub enum RecipientEntry { Orchestrator, Separator, AllAgents, Agent(AgentId) }
  pub enum CoordinationAction {
      ApproveProposal { revision: u64, digest: Digest },
      ApproveWorkflowPromotion(WorkflowPromotionReview),
  }
  ```
...
- Closed `RoomOperation` 只含 runtime 已实现的 `SendMessage`、`ProposeRoomBrief`、`AbandonIdleRequest`、`PersistWorkflowDraft`、`PromoteWorkflowDraft`、`AcquireResource`、`ReleaseResource` 与 `ApprovePermissionOnce`。Provider schema、capability map 与 Worker dispatch 必须与该集合一致：不存在 `CreateAgent`/`SteerActiveWork`/`SuspendWork`/`ResolveQueuedWork`/`UpdateCoordination`，也没有 registered operation 落入 generic unavailable fallback。Mid-work pivot 只是新的 model-authored `SendMessage` Request；runtime 不把它改写成 steering 或 queued-work disposition。Closed `RoomQuery` 同样只含 runtime 已实现的 `InspectWork`、`WaitForChange`、`ReadAgent`、`ObservePermissionPrompt`、`ReadWorkflowDraft` 与 `ReadContent`：不存在 `ReadCodebaseOutline`/`ReadArtifact`，也没有 registered query 落入 generic unavailable fallback。Artifact evidence 只经 `WorkSettlement` 暴露。
- `ProposeRoomBrief` 的 room 与 Orchestrator author 必须由 Worker 注入。它只接受：unlocked room；等于当前 brief revision 的 `expected_revision`；非空白的 goal/non-goals，其大小只受既有 Bus command/control frame 与 serialization limits 约束，不新增 per-field cap。Worker 自行分配下一 revision，经单一 writer durable 保存未确认 proposal，并由 `room_brief_proposal` 发布。Stale、locked 或 blank 输入 typed-reject，且零 Room Brief mutation（只记录标准 operation intent/rejection receipt）。该 operation 从不 confirm、lock 或选择 workflow step。
...
- `AbandonIdleRequest` 必须原子匹配 exact current Submitting/Active Request、agent、participant incarnation、observed semantic revision、optional provider turn 与 confirmed Idle；stale/wrong/non-Idle input typed-denied。Human confirmed UI/CLI 和 approved Orchestrator tool 进入同一个 Worker operation/receipt；成功只把该 Request 标为 `Abandoned` 并释放 agent，不修改 queue。Existing queued Requests retain their prior authorization and resume normal FIFO settlement；本计划不提供 queued-work disposition operation。
- 本计划不提供 `RetireParticipant`/`ReplaceParticipant`/`RequestHuman` operation，也不提供 human-task/attempt/lesson typed state；closed registry schema snapshot、capability map 与 Worker dispatch 都不包含它们，也不把它们留在 generic unavailable fallback 后面。任何 Orchestrator operation 都不删除 participant、message、request、assignment 或 artifact provenance；existing Human agent delete 行为不变。Active/uncertain work 在 reconcile 或 explicit authorized `AbandonIdleRequest` 之前不能复制投递。
- `MessageOrchestrator` 必须在一次 durable save 中持久化 Human-authored、Orchestrator-addressed、exact body 的 room message，并记录恰好一个 deduplicated `RoomWake::HumanMessage`。它不创建 Prompt/Request，不改变 agent FIFO、`AgentRecipients` 或 work settlement。Blank body、unknown room 或 Orchestrator 未启用时 typed-reject 且零 mutation。Context builder 从 journal 读取该 body，restart 后仍然可见。
- `ConfirmRoomBriefProposal` 必须由 Worker exact-match authoritative unlocked proposal、Human developer identity、room、proposal revision 与 canonical proposal digest；success 在同一次 durable save 中设置 approved revision/locked Room Brief、记录 single receipt 并返回 exact bound facts。Stale/mismatched identity/revision/digest、conflicting replay 或 chat-text lookalike typed-reject with no state change；exact retry/restart replay returns the prior receipt，不创建第二份 brief/confirmation。UI 没有 `set_room_brief` 或另一个 authoritative cache/write path。
- `CreateDeveloperWorkflowApproval` 必须由 Worker exact-match current Human developer identity、room、draft ID/revision/content digest、canonical workflow ID、current standard base digest 与 exact reviewed diff digest；success 原子持久化 immutable single-use approval and receipt。Stale/mismatched/replayed tuple typed-reject 或返回相同 prior receipt，绝不默认为最新 draft/base/diff；UI 不能直接调用 `WorkflowDraftStore::create_approval` 或建立第二份 approval SOT。
- `WorkflowPromotionReview` 只能由 Worker 以 `create_approval` 所用的同一 authoritative 计算产出。`approval_command()` 必须与 `create_approval` 的重算结果逐字段相等；draft revision 或 standard base 一旦变化，由旧 projection 派生的 command 必须 typed `StaleApproval`。Projection 字段均为 typed，不以 JSON 字符串或 operation digest 承载。
- UI proposal confirmation 必须派发 render 时 Worker projection 的 exact revision 与 digest。Revision 不变但 content 已改变时，以旧 digest 确认必须 typed-reject 且零 mutation。
- Generic `message.send`、`message.status`、bounded `wait` 与 room wake 必须共享 Worker-owned persisted Request/message settlement：send durable enqueue 后立即返回 identity，wait/status 在 CLI exit/restart 后仍查询同一 state，且同一次 Request settlement save 恰好产生可去重的 `RequestSettled` semantic fact 来 wake Orchestrator。Harness/content/UI 不得从 terminal、reply text 或 local timer 建立 polling/classification/notifier state；future `send --wait` 只能组合 existing send + wait。
- Permission observation 必须通过 `RoomQuery::ObservePermissionPrompt` / `agent permission AGENT` resolve stable room AgentId，并从同一 current prompt snapshot 返回 exact room/agent/incarnation/launch/terminal/session/public-pane/current Request/provider turn/content revision/prompt digest、current prompt text/facts、allowlist eligibility、fixed allowed responses 与 single-use fingerprint；query 不发送 bytes、不写 receipt、不解释 workflow。`ApprovePermissionOnce` / `agent approve-once AGENT --fingerprint FINGERPRINT --response allow-once` 只接受该 fingerprint 与 fixed enum；一个 Worker operation 在调用 native 前原子重验 room/agent/incarnation/launch/current Request/turn，native owner 的同一 compare-and-send handler 在写 bytes 前重验 terminal/pane/session/content revision/prompt digest 与 fixed response allowlist。成功 single-use consume fingerprint 并写 durable audit receipt；retry returns the same receipt without another write。Missing/stale/mismatched/unknown/risky/replayed input 返回 current facts 且 zero keys，不自动升级、联系 Human 或选择 workflow。Human 与 approved Orchestrator 使用完全相同的 typed operation；raw native `AgentSendKeys`、arbitrary key/string、broad shell 与 reusable permission authority 在其 CLI/schema/registry 中不可表达。
...
- [x] **完成**
- **目标**：在一个可独立编译/测试的 core owner 中交付 coordinator-thread 外的 room-bound model loop、private credential、DeepSeek adapter、Worker-owned logical append-only journal/operation ledger、reconstructible context、first-class participant/capability enforcement、closed non-coding Orchestrator tool registry、typed room operations/queries、Bus-owned on-demand complete-viewport/recent-tail terminal observation、semantic work settlement、participant incarnation lineage、bounded read-only workflow-draft/content access、pathless typed workflow-draft persistence、developer-approved typed standard-workflow promotion、content-agnostic `ROOM_AGENT_CONTENT_INTERFACE_V1`、trusted `TRUSTED_ROOM_ASSIGNMENT_V1` producer/discovery/read-only verifier、resource lease、factual permission observation/fingerprint + atomic approve-once guard、Human UI 可消费的 exact Room Brief confirmation / immutable workflow approval commands 与 durable receipts、Human→Orchestrator room message command、Worker-typed workflow promotion review projection 与 typed UI reads，以及 test-only in-process seams；不包含 production wording或 workflow 语义。
...
- **blocked-by**：无
- **produces**：`ROOM_AGENT_CORE_CONTRACT`（包含 closed non-coding tool registry、capability、operation/query、workflow draft/promotion、`ConfirmRoomBriefProposal` / `CreateDeveloperWorkflowApproval` / `MessageOrchestrator` typed command + durable replay-safe receipt seams、`WorkflowPromotionReview` projection、typed `BusState` UI reads、Task-2-usable test Worker command harness、single settlement SOT、persistence、lifecycle、permission、`ROOM_AGENT_CONTENT_INTERFACE_V1`、`TRUSTED_ROOM_ASSIGNMENT_V1` 与 test-seam contracts）
- **consumes**：无
...
- **约束**：所有 model tools sequential；model/provider HTTP 不占用 coordinator thread；Worker 是唯一 durable mutation writer；persist immutable intent before external effect and result/receipt immediately after；room/root/participant IDs injected by capability；Orchestrator registry/schema/dispatcher is closed and never inherits internal `BusCommand`、dev CLI、coding harness or content-declared tools；model has no shell/process、arbitrary patch/edit/write-file、Git、raw filesystem/paths or repository-general mutation，and all source/test/plan/unrelated-doc implementation/review/patch judgment is delegated by model-authored assignment to coding agents；bounded workflow-draft/content reads remain typed and read-only，and no codebase outline or artifact body query exists；repository mutation is expressible only as pathless `PersistWorkflowDraft` under canonical `.bus/temp` or pathless `PromoteWorkflowDraft` for the one `.bus/standard` target exact-bound to a single-use developer approval over reviewed draft/base/content/diff digests；both fail closed on traversal/symlink/reparse/case escape and can never reach non-workflow paths；workflow lint never chooses next action；`AgentRecipients` remains coding-terminal-only；role matrix deny-by-default；risk/unknown permission returns typed denial and only a later model call may request Human；`BackgroundPending` remains progress、trusted same-session continuation rebinds、ordinary correlated final settles only its provider Request、later provider activity/artifact are separate facts，and runtime never infers higher-level completion from text；Human confirmed UI/CLI and approved Orchestrator model share exact `AbandonIdleRequest`，which validates current/Idle/incarnation/revision/turn facts、abandons only the named Request and never chooses queued-work disposition；the closed registry、capability map and Worker dispatch contain exactly the runtime-implemented operations，with no `CreateAgent`/`SteerActiveWork`/`SuspendWork`/`ResolveQueuedWork`/`UpdateCoordination`/`RetireParticipant`/`ReplaceParticipant`/`RequestHuman` and no human-task/attempt/lesson typed state；coding agents are pre-existing Human-created room participants；semantic revision changes only with facts；wait/read/status use the running process without build；terminal observation is a Bus CLI/Worker-owned on-demand query over the existing native pane path：Bus `agent read AGENT --source visible` returns the complete same-revision viewport/dimensions and rejects `--lines`；Bus `agent read AGENT [--source recent] --lines N` requires positive N、rejects a bare/missing-N recent request、passes N unchanged and returns the last N rows or fewer only on actual scrollback exhaustion with requested/returned/available-or-exhausted/revision facts。Native Herdr `AgentRead`/`PaneRead` preserve `lines=None` and the existing recent-80 compatibility default，while explicit `Some(N)` has no 200/64-KiB/1,000-line or other maximum policy clamp。No offset/page/continuation token or silent truncation exists；the source-proven 16-MiB control ceiling remains an explicit no-partial-text error，and only the model may respond by choosing another N。Human and approved Orchestrator use the same capability，the model harness neither captures nor stores it，and only the model chooses whether the evidence changes workflow/recovery；content read is model-authored and cannot change tool grants；trusted assignment producer/directory discovery/read-only verifier/prompt correlation are harness-owned participant-delivery facts，not a result/workflow envelope，and verifier output never selects skill/fallback/progression；no live network in tests；reasoning content not rendered/logged；key reloaded by generation before call；no production wording and `production` content remains typed unavailable until successor bundle validation succeeds
  Task 1 additionally owns the only Worker mutations for exact initial proposal confirmation and standard-workflow approval creation：ConfirmRoomBriefProposal exact-matches authoritative Human/room/proposal revision/digest before atomically locking/persisting RoomBrief and its receipt；CreateDeveloperWorkflowApproval exact-matches authoritative developer、draft ID/revision/content digest、workflow ID、current standard base digest and reviewed diff digest before atomically persisting an immutable approval/receipt。Both survive restart；exact retries return the same receipt，while stale/conflicting/replayed inputs fail closed without mutation。Task 2 receives only these command/projection/receipt seams and cannot own another store or validator。Generic message send returns durable message/request identity immediately after enqueue；wait/status query that same persisted settlement after CLI exit/restart，and the same durable Request settlement emits RequestSettled to wake the Orchestrator。No harness/content/UI polling loop、terminal classifier、workflow notifier or second settlement SOT is allowed；future send-with-wait is composition only。
  The 2026-09-16 Task 2 review repair adds only objective seams to Task 1：
  - `MessageOrchestrator` persists the Human's exact body as a Human-authored、Orchestrator-addressed room message plus one deduplicated `RoomWake::HumanMessage` in one durable save。It creates no Prompt/Request，and the Orchestrator context still includes the body after restart。
  - `WorkflowDraftMutation::Create` binds an immutable optional target `WorkflowId`。Worker publishes typed `WorkflowPromotionReview` from the same computation that `create_approval` recomputes，so `approval_command()` is exactly what the validator accepts。
  - `BusState` exposes typed reads for proposal revision/digest、confirmation receipt、promotion reviews、approval receipts、room messages and work settlement。
  - A `#[cfg(test)] pub(crate)` Worker command harness applies `BusCommand`/`RoomOperation` through the real Worker handlers and returns the published snapshot，so Task 2 tests never fabricate Worker state。
  - `RoomOperation::ProposeRoomBrief { expected_revision, goal, non_goals }` is the single Room Brief proposal producer（decision 19）：
    - It reuses `Coordinate`；room and Orchestrator author are Worker-injected，and the model supplies no path or author。
    - Worker requires an unlocked room、`expected_revision` equal to the current brief revision，and nonblank goal/non-goals；their size is bounded only by the existing Bus command/control frame and serialization limits that already govern typed operations，with no per-field cap。It allocates the next revision itself and durably stores the unconfirmed proposal through the single Worker writer，so `room_brief_proposal` publishes it。
    - Stale、locked or blank input is typed-rejected with zero Room Brief mutation；only the standard operation intent/rejection receipt is recorded。
    - It never confirms、locks or chooses a workflow step。`TestWorkerHarness::orchestrator_operation` settles it through the same real path。
  - `CreateAgent`、`SteerActiveWork`、`SuspendWork`、`ResolveQueuedWork`、`UpdateCoordination`、`RetireParticipant`、`ReplaceParticipant`、`RequestHuman`、the `ReadCodebaseOutline` and `ReadArtifact` queries、`OutlineCursor`，their schema entries and any capability left without an operation are deleted rather than left behind the generic unavailable fallback。
  - `HumanTaskId` may remain defined in `src/bus/orchestrator/types.rs` as an inert、unexposed type with no producer、schema、dispatch、capability、state or runtime behavior。Task 2 deletes all of its UI uses。Deleting the harmless orphan definition itself is out of scope for this plan and is not a follow-up task。

  Task 1 adds no attempt/lesson/human-task/to-do/lifecycle-provenance state or projection。
- **验收闸门**：[TASK_LOCAL] `just test-one room_orchestrator_core_` + `just test-one agent_approve_once_` + `python3 -m unittest scripts.test_run_launcher scripts.test_bus_dev_acceptance scripts.test_skill_migration_contract` 均 exit 0 and select nonzero tests；the stable Rust prefixes include exact named cases for provider normalization/error classes、key redaction、closed registry/schema exact snapshot、unknown/extra tool and field rejection before Worker command、absence of shell/exec/process/arbitrary-patch/edit/write-file/Git/raw-filesystem/generic-coding-harness/internal-command bridges even when prompt/content/SOP asks、source/test/plan/unrelated-doc targets unreachable through every model tool、bounded workflow-draft/content read allowlist、coding-agent delegation path、“PersistWorkflowDraft” create/revise receipts、opaque ID/fixed “.bus/temp” derived target、128-KiB/UTF-8/schema/revision guards、draft-to-standard/other-root/traversal/symlink/reparse/case-escape/no-partial-write denial、immutable developer approval creation only from Human review surface、“PromoteWorkflowDraft” exact approval/draft/base/content/diff binding、canonical “.bus/standard” derived target、missing/forged/reused/stale/mismatched approval denial、non-workflow bytes unchanged、single-writer journal replay/compaction、operation crash windows/dedupe/uncertain reconcile、role allow/deny/forged/revoked/cross-room cases、author/recipient delivery classes、Idle-with-owned-request、semantic revision dedupe、outer→request→operation correlation、launch/hook readiness、Claude “BackgroundPending” plus trusted same-session continuation and later final、ordinary parent-final settlement without higher-level classification、Human and approved Orchestrator authorization of the same “AbandonIdleRequest” operation、no-authority/stale-incarnation/stale-revision/wrong-turn/non-Idle denial、successful “Abandoned” receipt with queue unchanged and later ordinary FIFO settlement、post-final/post-settlement provider activity persists and wakes without progression、filesystem artifact evidence、restart FIFO、stale queue、exact “agent read AGENT --source visible” and “agent read AGENT [--source recent] --lines N” parser/help forms、bare/missing-N Bus recent rejection、stable room-AgentId resolution、native public-pane selection、visible complete-viewport text with same-snapshot rows/columns and visible-with-lines rejection、recent positive-N requirement and exact forwarding for N below/equal/above 1,000、native Herdr agent/pane optional-lines compatibility including “lines=None”/recent-80 and visible-with-lines、all existing “PaneReadResult” fields preserved with additive observation facts、all direct schema/handler/subscription/headless literals updated、recent returned-N-or-actual-exhaustion semantics、requested/returned/available-or-exhausted/content-revision facts、absence of offset/page/continuation schema、16-MiB control-ceiling explicit no-partial-text error、no 200-line/64-KiB/1,000-line maximum for explicit N or silent clamp、missing/stale/mismatched pre/post runtime identity returning no uncorrelated text、Human/Orchestrator parity、read-only classification and zero status/revision/queue/focus/bytes/journal changes、non-building concurrent waits、resource lease、approve-once request classification、three independently audited safe-prompt approvals、atomic permission race rejection、Worker stale-launch denial before any native call、native terminal/pane/session/revision/digest race denial、“ROOM_AGENT_CONTENT_INTERFACE_V1” absent/valid/malformed/digest/layering/model-authored-read cases、“TRUSTED_ROOM_ASSIGNMENT_V1” record/active-pointer/frame publication before send、launch discovery env、production verifier “Verified”/“Absent”/“Invalid”、framed start exact-match、tampered initial “WrongPrompt”、trusted continuation、resume token refresh、replace/stale identity and forged-in-band cases、closed fake seams，以及 “no authorizing model/human decision, no workflow operation” while authorized capability settlement continues；launcher unittest separately preserves “dev” rebuild/failure behavior and covers non-building control、missing-running-binary fail-closed、both exact Bus read forms、bare/missing-N/zero/invalid recent、visible-with-lines rejection、exact large-N forwarding and absence of offset/page/continuation arguments；Bus acceptance unittest proves its live driver supplies an explicit recent N and the public guide documents read、permission、idle recovery and assignment-verifier contracts without a fixed Bus default；skill migration unittest proves both new “*-format.md” paths are in the exact inventory
  The same existing stable gate commands must now also select named Worker-path cases for exact initial proposal snapshot → ConfirmRoomBriefProposal → durable RoomBriefConfirmationReceipt，CreateDeveloperWorkflowApproval over exact developer/draft/revision/content/workflow/current-base/reviewed-diff facts，restart persistence of both receipts，exact retry idempotence，stale/conflicting/replayed rejection with zero mutation，and Task-2-facing receipt projections sourced only from Worker state。They also cover message.send immediate persisted message/request identity，message.status and wait querying the same settlement across waiter exit/Worker restart，one deduplicated RequestSettled wake from the same settlement save，and absence of any second polling/classification/notifier completion state。The same prefixes also select named Worker-path cases for：
  - “MessageOrchestrator”：exact-body persistence、a single deduplicated HumanMessage wake、zero Prompt/Request/FIFO/settlement change、Orchestrator-only success without “NoRecipients”、blank/unknown-room/disabled rejection with zero mutation，and the body preserved in context after restart；
  - workflow review：draft create with and without a target、revise unable to change the target、“WorkflowPromotionReview” equal field for field to the “create_approval” recomputation、an approval built from “approval_command()” succeeding，and draft-revision or external standard-base changes turning the prior command into typed “StaleApproval”；
  - proposal：a same-revision content change rejecting the previously displayed digest with zero mutation；
  - the test Worker command harness producing snapshots through the real handlers；
  - “ProposeRoomBrief”：schema/decode with Worker-injected room and author；a proposal from revision 0 and a successive proposal，each receiving the Worker-allocated next revision and appearing in the typed proposal read；stale revision、locked room and blank goal/non-goals rejected with zero Room Brief mutation；no auto-confirmation；and the same operation settled through “TestWorkerHarness::orchestrator_operation”；
  - a registry/schema/capability/dispatch snapshot proving the operation set equals the runtime-implemented variants，with “CreateAgent”/“SteerActiveWork”/“SuspendWork”/“ResolveQueuedWork”/“UpdateCoordination”/“RetireParticipant”/“ReplaceParticipant”/“RequestHuman” absent，and the query set equals the runtime-implemented queries，with “ReadCodebaseOutline”/“ReadArtifact” absent and no query reaching the generic unavailable fallback。

Task 1 上述 “Bus acceptance unittest” 仅指其 owner 内的 `scripts.test_bus_dev_acceptance`。
...
- [x] **完成**
- **目标**：配置 key 后，每个新 room 默认出现独立 Orchestrator participant。Room UI 完成以下交互：author-attributed conversation；经 Task 1 `MessageOrchestrator` 的 Human→Orchestrator message；派发 exact displayed revision/digest 的初始 proposal confirmation；Goal/Non-goals；来自 `WorkSettlement` 的 settlement labels；基于 Worker-typed `WorkflowPromotionReview` 的 immutable standard-workflow diff review/approval。同时删除所有没有 Worker 事实来源的 fake UI paths。
...
- **约束**：Task 2 只消费 Task 1 Worker seams，并遵守以下规则。
  - Recipients 与 authors：recipient order is Orchestrator, separator, All, agents；All selects coding agents only；Human/Orchestrator/Agent authors remain distinguishable。Room history interleaves Human↔Orchestrator room messages with prompt-grouped Request history by the shared monotonically allocated message/prompt identity，never by a timestamp or a client-owned ordering store。
  - Composer：selecting `@Orchestrator` dispatches Task 1 `MessageOrchestrator` with the exact submitted body and never enters PTY FIFO；the existing agent `Submit` is dispatched only when at least one coding agent is selected。The `@Orchestrator` toggle is composer input and need not survive restart；the sent message is the durable Worker fact。
  - Proposal：no automatic start before exact proposal confirmation。Proposals come only from the Orchestrator's `ProposeRoomBrief`；the Human requests brief changes through an ordinary `MessageOrchestrator` message。
  - Workflow promotion：the review renders the Worker-typed `WorkflowPromotionReview` identity、draft revision/content digest、current base digest and exact rendered diff before developer approval，and approval is an explicit separate action rather than chat text。
  - Settlement labels：labels map exhaustively from `WorkSettlement` typed fields；`Abandoned` renders as abandoned/recovered，and the UI never derives `wedged`。
  - Layout：narrow and short layouts keep composer and conversation reachable；legacy notes remain readable。
  - Deletions：delete the stringly `workflow_promotion_reviews` JSON path；`CoordinationAction::ResolveHumanTask` together with every `crate::bus::orchestrator::HumanTaskId` reference（including the hard-coded `HumanTaskId(0)`）and every fake human-task UI reference（the `request_human` to-do line and its action），so no consumer of `HumanTaskId` remains outside its Task-1-owned definition；the `RejectProposal` action；the to-do/lease/lifecycle-provenance/attempt/lesson panels；and every `DurableOperation.kind`/`intent_digest` or `serde_json::Value` read of Worker state。
  - Consumed seams：Task 2 consumes only Task 1 `ConfirmRoomBriefProposal`、`CreateDeveloperWorkflowApproval` and `MessageOrchestrator` commands，their durable receipts，typed `BusState` reads and the test Worker command harness，whose `orchestrator_operation` settles the real `ProposeRoomBrief` operation for proposal facts。Proposal actions carry the render-time Worker `(revision, proposal_digest)` and dispatch that tuple unchanged；workflow approval dispatches `WorkflowPromotionReview::approval_command()` from the rendered review。
  - Stale results：a stale Worker rejection renders as stale and requires re-review，with no local persistence、recomputation or automatic resubmission。The UI does not call set_room_brief or WorkflowDraftStore directly，does not persist or derive confirmation/approval，and owns no second validator/SOT。Declining a proposal、asking for help or reporting attempts is an ordinary room message。
  - Completion：the UI consumes semantic RequestSettled projections；it neither polls terminal/reply text for completion nor creates a completion classifier/notifier。Any future send-with-wait affordance invokes the same Task 1 send + wait composition only。
- **验收闸门**：[TASK_LOCAL] `just test-one room_orchestrator_ui_` exits 0 and selects nonzero tests。The stable prefix includes exact named cases for：
  - settings key redaction；
  - default activation on a room newly created through the Worker path：the Orchestrator recipient is listed first when a key is configured and absent otherwise，with no assertion that is true by construction；
  - recipient order、`All` excluding Orchestrator，and author distinction；
  - Human↔Orchestrator room messages interleaved with prompt-grouped history by the shared message/prompt identity；
  - an Orchestrator-only send dispatching “MessageOrchestrator” with the exact submitted body and no agent “Submit”，and an Orchestrator-plus-agent send dispatching both；
  - proposal confirmation dispatching the rendered revision/digest of a proposal produced by the harness's real “ProposeRoomBrief” operation，and a newer “ProposeRoomBrief” revision after display surfacing the Worker stale rejection without resubmission；
  - workflow review rendering the Worker-produced diff/digests and dispatching its “approval_command()”，and a draft/base change surfacing the Worker stale-approval rejection；
  - “Abandoned” labeled abandoned rather than wedged；
  - restart rendering both receipts from Worker state；
  - narrow and short layouts，color-blind，and scroll/history regressions。

  Every Worker-owned fact in those cases is produced through Task 1's test Worker command harness and real Worker handlers，and every rejection comes from the Worker。No UI test serializes or patches “BusState” JSON，hand-writes operations/leases/journal/approvals/receipts，or fabricates a rejection locally。The cases also prove that no client-owned RoomBrief/approval store、terminal/reply completion poller/classifier、workflow notifier or duplicate settlement source exists。
...
- Model loop: streaming/complete tool calls, finish reasons, retry/cancel, context overflow/compaction, token usage, no reasoning exposure.
...
- Human decision seams: Worker owns exact proposal projection and ConfirmRoomBriefProposal settlement，atomically locks/persists RoomBrief and a developer/revision/digest-bound receipt；Worker also owns CreateDeveloperWorkflowApproval over the exact developer、draft ID/revision/content digest、workflow ID、current standard base digest and reviewed diff digest，then persists an immutable single-use approval/receipt。Tests cover restart visibility、exact retry idempotence、stale/conflicting/replayed rejection and prove the UI has no direct store/validator/SOT path。Worker also owns `MessageOrchestrator`（exact body into durable room context、one deduplicated HumanMessage wake、no Prompt/Request/FIFO）and the typed `WorkflowPromotionReview` projection，whose `approval_command()` equals the `create_approval` recomputation。Tests cover Orchestrator-only messages without `NoRecipients`、the body preserved after restart、a stale projection after a draft/base change，and a same-revision proposal content change rejecting the displayed digest。`ProposeRoomBrief` tests cover Worker-allocated successive revisions，stale/locked/blank rejection with zero Room Brief mutation，and the absence of auto-confirmation。
- Generic notification round-trip: message.send durable enqueue returns message/request identity immediately；message.status and bounded wait query the same persisted settlement after waiter exit and Worker restart；the same Request settlement save emits one deduplicated RequestSettled fact that wakes the room Orchestrator。Tests prove no harness/content/UI terminal poller、reply classifier、workflow notifier or alternate completion state，and any future send-with-wait is only composition。
- Capability boundary: model schema/registry/dispatcher exact snapshot contains only typed room/process queries/operations；shell/exec/process、arbitrary patch/edit/write-file、Git、raw path/filesystem、generic coding-harness and internal `BusCommand`/dev-control bridges are absent，unknown/forged tool calls fail before Worker command even when prompt/content/SOP requests them。Bounded workflow-draft/content reads remain available without raw paths。The query schema equals the runtime-implemented set，with no codebase outline or artifact body query，and artifact evidence arrives only through `WorkSettlement`。Repository mutation is limited to two pathless workflow-specific variants：`PersistWorkflowDraft` writes only Bus-derived opaque `.bus/temp/<room-id>/<draft-id>.md` under 128-KiB UTF-8/structure/revision/atomic guards；`PromoteWorkflowDraft` writes only the canonical `.bus/standard/<workflow-id>.md` target bound by a single-use developer approval over exact draft revision/content、standard base and reviewed diff digests。Tests cover root canonicalization、absolute/relative traversal、symlink/reparse/case escape、missing/forged/reused/stale/mismatched approvals and partial-write failures；every source/test/plan/unrelated-doc/other non-workflow target remains unreachable and byte-identical。Repository implementation occurs only through explicit coding-agent assignment。
...
- Participant semantics: one Human plus Orchestrator and coding Agents have addressable identity, attributed messages, assignments, evidence, status and durable history；role allow/deny、grant/revocation、forged role 与 cross-room tests prove capability differences do not change first-class participation；Orchestrator never enters Provider/PTY FIFO，and a Human→Orchestrator message creates no Request；the registry has no retire/replace/RequestHuman operation，and no Orchestrator operation deletes incarnation、request or artifact provenance。
...
`just test` 运行 Task 1 与 Task 2 的 in-process scripted fake-provider 与 typed seam 测试；本计划不包含 real-process scripted acceptance driver（见决策 20）。
...
- [ ] **步骤 4**：中断并 resume Bus 后继续同一 room。
  - **预期结果**：Goal/Non-goals、journaled room messages（含 Human→Orchestrator body 与 attempt/lesson 内容）、participant incarnation、agent/reply/callback/artifact correlation 与 unresolved operations 恢复；没有重复创建或发送。Visual Idle、outer message complete 与 artifact existence 均只作为 evidence，Orchestrator 仍独立决定下一步。
