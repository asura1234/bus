...
description: Rebase the current Bus feature branch onto the latest explicit origin/main, resolve safe conflicts, verify the result, and update its remote branch with force-with-lease. Use for rebase, sync main, or update from origin/main requests.
---

# Rebase onto origin/main

This workflow targets the Bus fork's `origin/main`. It never substitutes
...
3. Inspect tracked and untracked changes. Do not stash, discard, or include
   ...
   a new rebase.
4. Fetch exactly `origin main` and verify `refs/remotes/origin/main` resolves.
...
9. Verify no rebase state remains, the worktree is clean, and the branch is
   based on `origin/main`. Run focused checks for resolved files; use `just ci`
